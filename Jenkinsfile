pipeline {
    ...
    stages {
        stage('git checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/AmarDahiwalkar/Ekart-Project.git'
            }
        }
    ...
    steps {
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t amardahiwalkar/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u amardahiwalkar -p ${dockerhubpwd}'}
                   sh 'docker push amardahiwalkar/ekart:latest'
                }
            }
        }
